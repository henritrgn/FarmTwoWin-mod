...
public class ProcedureOfStoneProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide() && event instanceof net.minecraftforge.event.level.BlockEvent.BreakEvent e
				&& e.getState().is(net.minecraft.world.level.block.Blocks.STONE)) {
			{
				entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).ifPresent(capability -> {
					capability.Count_stone_mined = entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).orElseGet(FarmtwowinModVariables.PlayerVariables::new).Count_stone_mined + 1;
					capability.markSyncDirty();
				});
			}
			FarmtwowinMod.LOGGER.info(entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).orElseGet(FarmtwowinModVariables.PlayerVariables::new).Count_stone_mined);
			if (entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).orElseGet(FarmtwowinModVariables.PlayerVariables::new).Count_stone_mined == 20) {
				FarmtwowinMod.LOGGER.info(entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).orElseGet(FarmtwowinModVariables.PlayerVariables::new).Count_stone_mined);
				{
					entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).ifPresent(capability -> {
						capability.Count_stone_mined = 0;
						capability.markSyncDirty();
					});
				}
				FarmtwowinMod.LOGGER.info(entity.getCapability(FarmtwowinModVariables.PLAYER_VARIABLES).orElseGet(FarmtwowinModVariables.PlayerVariables::new).Count_stone_mined);
				...
				if (Random < -2) {
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(Blocks.COAL_ORE).copy();
					...
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(Blocks.IRON_ORE).copy();
						_setstack.setCount(3);
						ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
					}
					...
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(Blocks.GOLD_ORE).copy();
					...
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(Blocks.DIAMOND_ORE).copy();
						_setstack.setCount(1);
						ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
					}
